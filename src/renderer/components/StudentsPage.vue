...
">
        ...
        <b-form-group>
            <b-input-group>
                ...
                <b-input-group-append>
                    <b-button variant="outline-secondary" class="btn-with-only-icon StudentPageScanIcon"
                              @click="openScanModal">
                        <QrIcon decorative></QrIcon>
                    </b-button>
                </b-input-group-append>
            </b-input-group>
        </b-form-group>
        ...
        >
            ...
            <template v-slot:cell(checkbox)="data" class="table-checkbox">
                <b-form-checkbox v-model="selected" :value="data.item.studentID"
                                 :key="data.item.studentID"></b-form-checkbox>
            </template>
            ...
            <template v-slot:table-busy="scope"
                      class="text-center text-primary my-2 d-flex justify-content-center align-items-center">
        ...
        <div class="d-flex  justify-content-between align-items-center">
            <div>
                <div v-if="selectMode" class="d-flex justify-content-start align-items-baseline">
                    <b-dropdown split @click="$wait(savePDF())" size="sm" id="dropdown-1"
                                class="mr-2 dropdown-with-icon">
                        <template slot="button-content">
                            <FileDownloadIcon class="top-1px"></FileDownloadIcon>
                            Печать в PDF
                        </template>
                        <b-dropdown-item @click="$wait(print())">
                            <PrinterIcon></PrinterIcon>
                            Распечатать
                        </b-dropdown-item>
                        <b-dropdown-item v-if="selected.length==1" @click="edit">
                            <EditIcon></EditIcon>
                            Изменить
                        </b-dropdown-item>
                        ...
                        <b-dropdown-item v-if="!viewSelected" @click="addAll">
                            <CheckAllIcon></CheckAllIcon>
                            Выделить все{{query.length>0?' найденные':''}}
                        </b-dropdown-item>
                        <b-dropdown-item @click="clearSelected">
                            <ClearCheckIcon></ClearCheckIcon>
                            Убрать выделение
                        </b-dropdown-item>
                    </b-dropdown>
...
<script>
    ...
    export default {
        ...
        components: {
            FileDownloadIcon,
            QrIcon,
            ScanningStudentCardModel,
            StudentDangerActionModel,
            AdAccIcon,
            PrinterIcon,
            EditIcon,
            DeleteIcon,
        ...
        methods: {
            ...
            reident() {
                this.$modal.show('danger-students-action', {
                    ...
                    callback: () => {
                        this.$wait(this.$store.dispatch('Students/reidentification', this.selected).then(() => {
                            this.$modal.hide('danger-students-action');
                            this.viewSelected = true
                        }), true, 0);
                    }
            ...
            async generatePDF() {
                ...
                for (let st in this.selected) {
                    console.log(this.$store.state.Students.students[this.selected[st]])
